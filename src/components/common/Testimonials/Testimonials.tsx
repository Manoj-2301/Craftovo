      ...
      <Container className="relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          ...
          <AnimateIn delay={0.2} className="flex gap-4 shrink-0 mb-8 md:mb-16">
            <button 
              onClick={scrollPrev}
              className="w-14 h-14 rounded-full glass-panel flex items-center justify-center text-text-main hover:bg-black/5 transition-colors"
            ...
            <div className="flex -ml-6">
              {testimonials.map((item, index) => (
                <div key={index} className="flex-[0_0_100%] min-w-0 md:flex-[0_0_50%] lg:flex-[0_0_40%] pl-6">
                  <div className="glass-panel p-10 md:p-12 rounded-[2rem] border-black/5 h-full flex flex-col justify-between">
                    <svg className="w-10 h-10 text-text-main/20 mb-8" viewBox="0 0 24 24" fill="currentColor">
                    ...
                    <p className="text-xl md:text-2xl text-text-main font-light leading-relaxed mb-12">
                      "{item.quote}"
                    </p>
                    <div className="flex items-center gap-4">
                      <div className="relative w-14 h-14 rounded-full overflow-hidden border border-black/10">
                        <Image src={item.avatar} alt={item.author} fill className="object-cover" />
                      </div>
                      <div>
                        <h4 className="font-outfit font-medium text-text-main text-lg">{item.author}</h4>
